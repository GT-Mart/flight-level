...
def create_all_sales_table(first_year, db, logger, fields):
    ...
    db.sql(
        "CREATE TABLE all_sales (product_id BIGINT, product_name VARCHAR, product_category VARCHAR, package_qty DOUBLE, sales_qty BIGINT, product_price DOUBLE, sales_price DOUBLE, category_pct DOUBLE, day_pct DOUBLE, sales_date TIMESTAMP, page BIGINT);"
    )
    ...
    while idx <= datetime.datetime.now().year:
        ...
        r1 = db.sql(
            f"SELECT 'SELECT {','.join(fields)},<page> FROM '|| name FROM sqlite_master where type = 'table' AND name like 'sales_{idx}%' ORDER BY name"
        ).fetchall()
        ...
        for table in tables:
            create_table = f"""
            INSERT INTO all_sales({','.join(fields)},page) {table.replace('<page>',str(page))}
            """
...
def create_all_fuel_table(first_year, db, logger, fields):
    ...
    db.sql(
        "CREATE TABLE all_fuel (dispenser VARCHAR,sales_date TIMESTAMP,fuel_type VARCHAR,fuel_sales DOUBLE,fuel_sales_discount DOUBLE,fuel_sales_paid DOUBLE,fuel_volume DOUBLE, page BIGINT);"
    )
    ...
    while idx <= datetime.datetime.now().year:
        logger.info(f"Processing year {idx}...")
        r1 = db.sql(
            f"SELECT 'SELECT {','.join(fields)}, <page> FROM '|| name FROM sqlite_master where type = 'table' AND name like 'fuel_{idx}%' ORDER BY name"
        ).fetchall()
        ...
        for table in tables:
            create_table = f"""
            INSERT INTO all_fuel({','.join(fields)},page) {table.replace('<page>',str(page))}
            """
